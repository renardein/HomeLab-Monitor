...
async function fetchBackupJobsPayload(req) {
...
router.fetchBackupJobsPayload = fetchBackupJobsPayload;

router.get('/jobs', checkAuth, async (req, res) => {
    try {
        const result = await getCachedOrFetch('backup_jobs_v4', req, () => fetchBackupJobsPayload(req));
        res.json(result);
    } catch (error) {
